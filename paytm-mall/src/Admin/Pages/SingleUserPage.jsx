...
<SimpleGrid columns={{base:1,sm:2,md:3,lg:3}} w='100%' mt='30px'>
          ...
          <Image border='1px solid gray' src={data.image1} alt="image" w='80px' h='80px'/>
          <Image className="hoverToIncreaseWidth" mt='70px' ml={{base:'10px',sm:'20px',md:'70px',lg:'80px'}}  src={data.image1} alt="image" w='200px' h='300px'/>
          </Box>
         <Box align='left'  bg='white' p='10px'>
              <Text fontSize='2xl' as='b'>{data.title}</Text>
              <br />
              <Text  fontSize='lg' as='b'>Brand: {data.brand}</Text>
              <br />
              <Text fontSize='sm' as='mark' >{data.Category}</Text>
          
              <Box display='flex'>   
              <Text as='b' fontSize='xl'>Original-Price: </Text> 
              <Text display='flex' justifyContent='center' alignItems='center' ml='20px' bg='red' borderRadius='2px' color='white' fontSize='xl'  width='120px' as='b' > ₹{data.MRP}</Text></Box>
              <Box mt='10px' display='flex'><Text as='b' fontSize='xl'>Discounted-Price: </Text>
              <Text fontSize='xl' display='flex' justifyContent='center' alignItems='center' ml='20px' bg='green' borderRadius='2px' color='white'  width='120px' as='b'> ₹{data.price}</Text>
